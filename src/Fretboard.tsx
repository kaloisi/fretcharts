...
import GuitarState, { ScalePattern, StringState } from './models/GuitarState';
...
class Fretboard extends React.Component<FretboardProps, FretboardState> {
  constructor(props: FretboardProps) {
    ...
    let key = keyName && KEYS[keyName] ? KEYS[keyName] : KEYS.C_MAJOR;
      ...
      for (let i = 0; i < savedScales.length; i += 1) {
        if (savedScales[i]) {
          ...
          if (args.length === 3) {
            ...
            if (scalePattern && positionNum < positions.length) {
    ...
    this.state = {
      key: key,
      scales: guitarState.scalePatterns,
  ...
  onNoteClick(toneState: any): void {
    ...
    return (
      <div>
        ...
        <div className="fret-board">{
          this.state.guitarState.strings.map((stringState: StringState) => {
            return (
                  <GuitarString 
                      beat={this.state.beat}
                      key={stringState.uid}
                      musicKey={this.state.key} 
                      value={stringState} 
                      onClick={(g : any) => this.onNoteClick(g)} 
                      scales={scales}
                  />
            )})
            }
