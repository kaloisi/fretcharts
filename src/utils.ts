export const TONES: string[] = ["C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B"];

const MAJOR_SEMITONS: number[] = [0, 2, 4, 5, 7, 9, 11];
const MINOR_SEMITONS: number[] = [0, 2, 3, 5, 7, 8, 10];


export interface MusicKey {
  name: string;
  startingSemitone: number;
  semitones: number[];
  tones: string[];
}

interface Position {
  rootStringNumber: number;
  positionName: string;
  name: string;
  abr: string;
  fretOffsets: number[][];
...
function buildKey(name: string, startingPos: number, semitones: number[])
: MusicKey {
  const tones: string[] = semitones.map(t => TONES[(startingPos + t) % TONES.length]);
  return {
    name,
...
function resolveNotesForScale(guitarState: any, fret: number, position: Position): Map<any, string> {
  const notes = new Map();
  ...
  for (let s = 0; s < offsets.length; s++) {
    ...
    for (let f = 0; f < stringOffsets.length; f++) {
...
export const KEYS: { [key: string]: MusicKey } = {
...
export const Utils = {
    ...
    getPositionsForString(string: number): Position[] {
        return Object.values(this.POSITIONS).filter((n: Position) => n.rootStringNumber === string);        
    },
    getDefaultPositionForString(stringNumber: number): Position | undefined {
        const availablePositions = this.getPositionsForString(stringNumber);

        if (!availablePositions || availablePositions.length < 1) {
          return undefined;
        }

        return availablePositions.find((n: Position) => n.default === true);
    }
};
